    ...
    <v-flex xs12>
        <v-toolbar >
            <v-layout row wrap>
                ...
                <v-spacer></v-spacer>
                <v-btn flat @click="$router.push('/about')" class="hidden-xs-only">About</v-btn>
                <v-btn flat @click="$router.push('/')" class="hidden-xs-only">Home</v-btn>
                <v-btn flat @click="$router.push('/login')" class="hidden-xs-only">{{user ? 'Dashboard' : 'Login'}}</v-btn>
            </v-layout>
        </v-toolbar>
        <v-navigation-drawer
                v-model="sidebar"
